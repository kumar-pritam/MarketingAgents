  ...
  function formatValueForPDF(value: any): string {
    ...
    if (Array.isArray(value)) {
      ...
      return value.map(item => {
        if (typeof item === 'string') return `<span style="display: block; margin: 4px 0;">• ${item}</span>`;
        if (typeof item === 'object') return `<span style="display: block; margin: 4px 0; padding-left: 12px;">• ${JSON.stringify(item, null, 2).replace(/\n/g, '<br>')}</span>`;
        return '';
      }).join('');
    }
    if (typeof value === 'object') {
      return Object.entries(value).map(([k, v]) => `<div style="margin: 4px 0;"><strong>${k}:</strong> ${v}</div>`).join('');
    }
    return String(value);
  }
                  ...
                  {showDebug && (
                    <div className="debug-content">
                      <div className="debug-source">
                        ...
                        <span className={`badge badge-${debugInfo.source}`}>{debugInfo.source}</span>
                      </div>
                      {debugInfo.filled_sections?.length > 0 && (
                        <div className="debug-missing">
                          <strong>LLM filled:</strong>
                          <ul>
                            {debugInfo.filled_sections.map((field: string, i: number) => (
                              <li key={i}>{field}</li>
                            ))}
                          </ul>
                        </div>
                      ...
                      {debugInfo.missing_fields?.length > 0 && (
                        <div className="debug-missing">
                          <strong>Missing (filled with defaults):</strong>
                          <ul>
                            {debugInfo.missing_fields.map((field: string, i: number) => (
                              <li key={i}>{field}</li>
                            ))}
                          </ul>
                        </div>
                      )}
                      {debugInfo.llm_returned && (
                        <details className="debug-raw">
                          <summary>Raw LLM response (first 500 chars)</summary>
                          <pre>{debugInfo.llm_returned}</pre>
                        </details>
                      )}
                      {debugInfo.error && (
